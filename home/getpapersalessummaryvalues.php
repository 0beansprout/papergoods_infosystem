...
try {
	...
	$STH = $conn->prepare('SELECT * from employees WHERE EmpAuthCode=:param_auth');
	...
	if($emprow = $STH->fetch()) {
		...
		$STH = $conn->prepare('SELECT count(invoiceID) as transactioncount, sum(invoiceTotalCost) as totalprice from invoice where invoiceDate between DATE_SUB(CURDATE(), INTERVAL 7 DAY) AND CURDATE()');
		//
		$STH->execute();
		$result = $STH->fetch();
		$tmpCheckCount = $result['transactioncount'];
		$tmpCheckTotal = $result['totalprice'];
		
		$arr = array( "Status" => "okay",
			"TransactionCount" => "$tmpCheckCount", 
			"OrderTotal" => "$tmpCheckTotal" );
	}
