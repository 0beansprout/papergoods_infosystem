...
function Header()
{
    ...
    $this->Cell(65);
    // Title
    $this->Cell(50,10,'Product Report',0,0,'C');
    // Line break
    $this->Ln(20);
}
...
try {
	...
	$stmt = $conn->query('SELECT * from product ORDER BY proID');
	...
	while($row = $stmt->fetch()) {
       ...
       // $pdf->Cell(40,7,$empFullName,0);
        $pdf->Cell(20,7,$row['proID'],0);
        $pdf->Cell(25,7,$row['proName'],0);
        $pdf->Cell(40,7,$row['proBrand'],0);
        $pdf->Cell(25,7,$row['proQty'],0);
        $pdf->Cell(25,7,$row['proPrice'],0);
